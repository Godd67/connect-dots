    ...
    addPair() {
        ...
        if (this.hardMode) {
            ...
            if (this.grid.paths.length < 4) {
                ...
                const startR = start[0] < halfR ? 0 : 1; // 0=Top, 1=Bottom
                const startC = start[1] < halfC ? 0 : 1; // 0=Left, 1=Right
            ...
            if (this.grid.paths.length < 2) {
                const minDistance = Math.floor(Math.max(this.grid.rows, this.grid.cols) * 1 / 2);
                candidateList = candidateList.filter(([r, c]) => {
        ...
        while (q.length > 0) {
            ...
            for (const [dr, dc] of dirs) {
                ...
                if (this.grid.isValid(nr, nc)) {
                    ...
                    const maxStraight = Math.floor(Math.max(this.grid.rows, this.grid.cols) * 1 / 3);
